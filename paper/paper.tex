...
\section{gaussian processes and the celerite model}
	...
	\subsection{The \celerite model}
		...
		\begin{equation}
			k_\alpha(x_n, x_m) = \sigma_n^2 \delta_{nm} + \sum_{j=1}^J a_j e^{-c_j\tau_{nm}}
		\end{equation}
		where $\bvec{\alpha} = (a_1...a_j, c_1...c_j)$, $\sigma_n^2$ is the variance of the white noise, and $\tau_{nm} = |x_n-x_m|$. 
...
\section{2d gaussian processes}
	...
	In two dimensions the kernel function depends on two coordinates. Whereas above we used $\bvec{x}$ to refer to the single coordinate, we will now use
	\begin{equation}
		\bvec{t} = \expandvec{t_1}{t_N}
	\end{equation}
	to refer to the first coordinate, and
	\begin{equation}
		\bvec{u} = \expandvec{u_1}{u_M}
	\end{equation}
	to refer to the second coordinate. In most cases, we will demand that $M << N$. We then redefine $\bvec{x}$ to be the list of points
	\begin{equation}
	...
	\begin{equation}
		K_\bvec{\alpha\beta} = \Sigma + 
			\begin{bmatrix}
				r_{11}\circ Q_\bvec{\alpha} & ... & r_{1N}\circ Q_\bvec{\alpha} \\
				\vdots & \ddots & \\
				r_{N1}\circ Q_\bvec{\alpha} & ... & r_{NN}\circ Q_\bvec{\alpha}
			\end{bmatrix}
	\end{equation}
	where $[\Sigma]_{ij} = \sigma_i^2\delta_{ij}$ and
	\begin{equation}
		r_{pq}\circ Q_\bvec{\alpha} = 
			\begin{bmatrix}
				r_{pq}\circ q_{11} & ... & r_{pq}\circ q_{1M} \\
				\vdots & \ddots & \\
				r_{pq}\circ q_{M1} & ... & r_{pq}\circ q_{MM}
			\end{bmatrix}
	\end{equation}
	If the operator $\circ$ represents multiplication, we have 
	\begin{equation}
		K_\bvec{\alpha\beta} = \Sigma + R_\bvec{\beta} \otimes  Q_\bvec{\alpha}
	\end{equation}
